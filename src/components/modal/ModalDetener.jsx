...
function ModalDetener(props){
    ...
    const handleClickDetener = async(e) => {
        ...
        if (token.status == 200){
            ...
            const stop = await axios.post(config.REACT_APP_API_URL + '/stop',payloadToken,headers)
            if(stop.data == 1){
                setEstadoPreload(false)
                setTituloModal("MENSAJE")
                setTextoModal("Operacion cerrada con exito")
                setEstadoModal(true)
                props.actualizarLog()
                ...
                setEstadoPreload(false)
                setTituloModal("MENSAJE")
                setTextoModal("Ocurrio un error al cerrar la operacion")
                setEstadoModal(true)
                props.actualizarLog()
                props.re.current.scrollTop = props.re.current.scrollHeight
            }
            ...
            <div className='overlay'>
                <div className='container-modal'>
                    ...
                    <div className='contenido-modal'>
                        <h1>Detener Operacion</h1>
                        <div className='btn-salir'>
                            <button onClick={ handleClickDetener }>Aceptar</button>
